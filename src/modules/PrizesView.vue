...
// Converte data ISO del server nel formato richiesto dall'input datetime-local (YYYY-MM-DDThh:mm)
...
// Format per la visualizzazione in tabella
...
const salvaModifiche = async () => {
  if (!premioInModifica.value || !premioInModifica.value.nome.trim() || !premioInModifica.value.termina) {
    return alert("Il nome del premio e la data di scadenza sono obbligatori.")
  }

  try {
    const url = isNew.value ? '/api/prizes' : `/api/prizes/${premioInModifica.value.id}`
    const method = isNew.value ? 'POST' : 'PATCH'
    
    // Preparazione payload per il backend (converto la data locale in ISO)
    const payload = {
    ...
    if (res.ok) {
      await caricaPremi() // Rinfresca la tabella
      barOpen.value = false
      selectedId.value = null
      premioInModifica.value = null
    } else {
      const errore = await res.json()
      alert("Errore durante il salvataggio: " + (errore.message || "Richiesta non valida"))
    }
  } catch (error) {
...
<template>
  <div class="dashboard-page">
    <header class="navbar-panel">
      <div class="brand">🎁 GESTIONE <span class="accent">CATALOGO PREMI</span></div>
      <div class="controls-group">
        <button @click="preparaNuovoPremio" class="btn-primary">➕ Aggiungi Nuovo Premio</button>
      </div>
    </header>
    ...
    <div class="main-content">
          ...
          <table @mouseleave="activeColIndex = null">
            <thead>
              <tr>
                ...
                <th :class="{ 'on-hover': activeColIndex === 3 }">COSTO (PT)</th>
                <th :class="{ 'on-hover': activeColIndex === 4 }">SCADENZA</th>
              </tr>
            </thead>
            <tbody>
              <tr 
                v-for="p in premi" :key="extractId(p.self)"
                :class="{ 'selected': selectedId === extractId(p.self), 'inactive-row': !p.attivo }"
                @click="selezionaPremio(p)"
              >
                <td @mouseover="handleMouseOver(0)">
                  <span :class="['status-badge', p.attivo ? 'badge-active' : 'badge-inactive']">
                    {{ p.attivo ? 'Attivo' : 'Non Attivo' }}
                  </span>
                </td>
                <td @mouseover="handleMouseOver(1)"><strong>{{ p.nome }}</strong></td>
                <td @mouseover="handleMouseOver(2)" class="cell-descrizione">{{ p.descrizione }}</td>
                <td @mouseover="handleMouseOver(3)" class="cell-costo">{{ p.costo }}</td>
                <td @mouseover="handleMouseOver(4)">{{ formatDateVisual(p.termina) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <aside class="sidebar-panel panel bar" :class="{ 'open': barOpen }">
        <div v-if="premioInModifica" class="settings-content">
          ...
          <div class="scroll-area form-container">
            ...
            <div class="form-group">
              ...
              <p class="hint">Determina se gli utenti possono attualmente vedere e riscattare questo premio.</p>
              ...
              <div class="modules-checkbox-list">
                <label class="checkbox-item">
                  ...
                  <div class="checkbox-label-text">
                    ...
                    <span class="mod-desc">Rendi il premio disponibile da subito</span>
                  </div>
                </label>
              </div>
            </div>
        ...
        <div v-else class="empty-state">
          Seleziona un premio dalla tabella per configurarlo o clicca su "Aggiungi Nuovo Premio".
        </div>
      </aside>
    </div>
  </div>
...
/* Adattamento colonne per tabella premi */
th:nth-child(1) { width: 120px; }
th:nth-child(3) { width: 35%; }
th:nth-child(4) { width: 110px; }

tbody td { padding: 15px; border-bottom: 1px solid #edf2f7; font-size: 0.85rem; color: #4a5568; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; align-content: center; }
...
.cell-costo { font-weight: bold; color: #d69e2e; }
...
.inactive-row .cell-costo { color: #b7791f; opacity: 0.7; }
...
.badge-inactive { background: #fed7d7; color: #742a2a; border: 1px solid #feb2b2; }
...
/* --- CHECKBOX LIST (Ripreso fedelmente dal tema originale) --- */
...
.checkbox-item:hover { background: #f0fff4; border-color: #c6f6d5; }
