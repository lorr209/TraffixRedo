...
const caricaModuli = async () => {
  try {
    ...
    // Fallback visivo per test se l'API non è raggiungibile
    moduli.value = [
      { self: "/api/modules/1", nome: 'Anagrafica', descrizione: 'Gestione dati sensibili utenti', percorso: '/anagrafica', attivo: true },
      { self: "/api/modules/2", nome: 'Smart City Map', descrizione: 'Visualizzazione sensori su mappa 3D', percorso: '/mappa', attivo: true },
      { self: "/api/modules/3", nome: 'Report System', descrizione: 'Generazione PDF e statistiche', percorso: '/report', attivo: false }
    ]
  }
}
...
// --- 5. CHIAMATE API (PATCH) ---
const salvaModifiche = async () => {
  if (!moduloInModifica.value) return

  try {
    // Il backend accetta solo l'aggiornamento dello stato "attivo"
    const payload = {
    ...
    if (res.ok) {
      await caricaModuli() // Rinfresca la tabella con i dati reali del server
      barOpen.value = false
      selectedId.value = null
      moduloInModifica.value = null
    } else {
      const errore = await res.json()
      alert("Errore durante l'aggiornamento: " + (errore.message || "Richiesta non valida"))
    }
  } catch (error) {
...
<template>
  <div class="dashboard-page">
    <header class="navbar-panel">
      <div class="brand">⚙️ GESTIONE <span class="accent">MODULI DI SISTEMA</span></div>
    </header>
          ...
          <table @mouseleave="activeColIndex = null">
            <thead>
              <tr>
                ...
                <th :class="{ 'on-hover': activeColIndex === 2 }">DESCRIZIONE</th>
                <th :class="{ 'on-hover': activeColIndex === 3 }">PERCORSO (ROUTING)</th>
              </tr>
            </thead>
            <tbody>
              <tr 
                v-for="m in moduli" :key="extractId(m.self)"
                :class="{ 'selected': selectedId === extractId(m.self), 'inactive-row': !m.attivo }"
                @click="selezionaModulo(m)"
              >
                <td @mouseover="handleMouseOver(0)">
                  <span :class="['status-badge', m.attivo ? 'badge-active' : 'badge-inactive']">
                    {{ m.attivo ? 'Operativo' : 'Disattivo' }}
                  </span>
                </td>
                <td @mouseover="handleMouseOver(1)"><strong>{{ m.nome }}</strong></td>
                <td @mouseover="handleMouseOver(2)" class="cell-descrizione">{{ m.descrizione }}</td>
                <td @mouseover="handleMouseOver(3)"><code class="path-code">{{ m.percorso }}</code></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-if="moduloInModifica" class="settings-content">
          ...
          <div class="scroll-area form-container">
            ...
            <div class="info-alert">
              ...
              Nome, descrizione e percorsi di routing sono definiti a livello di core e non possono essere modificati da questa interfaccia.
            </div>
            ...
            <div class="form-group">
              ...
              <p class="hint">Abilita o disabilita globalmente l'accesso a questo modulo.</p>
              
              <div class="modules-checkbox-list">
                <label class="checkbox-item" :class="{ 'is-active': moduloInModifica.attivo }">
                  ...
                  <div class="checkbox-label-text">
                    ...
                    <span class="mod-desc">Gli utenti autorizzati potranno accedervi</span>
                  </div>
                </label>
              </div>
            </div>
...
tbody tr { cursor: pointer; transition: background-color 0.2s, opacity 0.2s; }
tbody tr:hover { background-color: #f7fafc; }
.inactive-row td { color: #a0aec0; }
.path-code { background: #edf2f7; padding: 3px 6px; border-radius: 4px; font-family: monospace; font-size: 0.8rem; color: #4a5568; }
...
.badge-inactive { background: #e2e8f0; color: #4a5568; border: 1px solid #cbd5e0; }
...
.info-alert { background: #fffff0; border: 1px solid #f6e05e; color: #744210; padding: 12px; border-radius: 8px; font-size: 0.85rem; margin-bottom: 20px; line-height: 1.4; }
...
.form-group input, .form-group textarea { width: 100%; padding: 10px; border: 1px solid #cbd5e0; border-radius: 8px; font-size: 0.85rem; color: #2d3748; background: #f7fafc; transition: all 0.2s; font-family: inherit; resize: none; box-sizing: border-box; }
...
/* --- CHECKBOX LIST --- */
.modules-checkbox-list { display: flex; flex-direction: column; gap: 8px; margin-top: 10px; }
.checkbox-item { display: flex; align-items: flex-start; padding: 12px 10px; border: 1px solid #edf2f7; border-radius: 8px; background: #f7fafc; cursor: pointer; transition: all 0.2s; }
.checkbox-item.is-active { background: #f0fff4; border-color: #c6f6d5; }
