<script setup>
	...
	const calcolaRestanti = computed(() => {
		const totaleAccanto = tipologieAggiuntive.value.reduce(
			(a, b) => a + (Number(b.percentuale) || 0),
			0,
		);
	...
	const rimuoviTipologia = (index) => {
		tipologieAggiuntive.value.splice(index, 1);
	};
	...
	const updateMapElements = () => {
		...
		const startMs =
			tInizioMs.value +
			(rangeInizio.value / 100) * (tFineMs.value - tInizioMs.value);
		const endMs =
			tInizioMs.value +
			(rangeFine.value / 100) * (tFineMs.value - tInizioMs.value);
			...
			sensoriFissi.value.forEach((s) => {
				...
				if (itemTime >= startMs && itemTime <= endMs) {
					...
					L.circle([s.lat, s.lon], {
						...
						fillColor: "#f56565",
						fillOpacity: 0.12,
						radius: raggioBase * 2.2,
					}).addTo(markersLayer.value);
					L.circle([s.lat, s.lon], {
						color: "transparent",
						fillColor: "#e53e3e",
						fillOpacity: 0.28,
						radius: raggioBase * 1.4,
					}).addTo(markersLayer.value);
					L.circle([s.lat, s.lon], {
						color: "#c53030",
						weight: 1,
		...
		if (layers.value.heatmap) {
			...
			Object.keys(gruppiVeicoli).forEach((idVeicolo) => {
				const puntiOrdinati = gruppiVeicoli[idVeicolo].sort(
					(a, b) => getLocalTime(a.data) - getLocalTime(b.data),
				);
				const coordinateLinea = puntiOrdinati.map((p) => [p.lat, p.lon]);

				if (coordinateLinea.length > 1) {
					L.polyline(coordinateLinea, {
						color: "#3182ce",
						weight: 3,
						opacity: 0.8,
						dashArray: "6, 6",
				...
				if (puntiOrdinati.length > 0) {
					...
					L.circleMarker([ultimoPunto.lat, ultimoPunto.lon], {
						radius: 7,
						color: "#1a365d",
						fillColor: "#63b3ed",
						fillOpacity: 1,
	...
	const togglePlay = () => {
		...
		if (isPlaying.value) {
			if (rangeFine.value >= 100) {
				rangeInizio.value = 0;
				rangeFine.value = 5;
			}
			const step = () => {
				if (!isPlaying.value || rangeFine.value >= 100) {
	...
	watch([rangeInizio, rangeFine, dataLimiteInizio, dataLimiteFine], () => {
		updateMapElements();
	});
	onMounted(initMap);
</script>
	...
	<div class="dashboard-page">
		...
		<div class="main-content">
			...
			<aside class="sidebar-panel panel">
				...
				<div class="fleet-box">
					<div
						v-for="(t, index) in tipologieAggiuntive"
						:key="t.id"
						class="fleet-item"
					>
						<div class="item-header">
							...
							/>
							<button @click="rimuoviTipologia(index)" class="btn-delete">
								x
							</button>
						</div>
						<div class="t-inputs">
							<label
								...
								<input
									type="number"
									v-model.number="t.percentuale"
									min="0"
									max="100"
							/></label>
							<label
								>CO₂ <input type="number" v-model.number="t.emissione" min="0"
							/></label>
						</div>
					</div>
					...
					<div class="fleet-item default-item">
						<div class="restanti-label">
							Veicoli Restanti: <strong>{{ calcolaRestanti }}%</strong>
						</div>
