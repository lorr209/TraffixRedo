					...
					<div class="slider-container">
						<div class="labels">
							...
							<span class="range-info"
								>{{ formatTime(rangeInizio) }} -
								{{ formatTime(rangeFine) }}</span
							>
							<span>Oggi</span>
						</div>
			...
			<aside class="sidebar">
				...
				<div class="fleet-box">
					<div v-for="t in tipologieAggiuntive" :key="t.id" class="fleet-item">
						...
						<div class="t-inputs">
							<label
								>% <input type="number" v-model.number="t.percentuale"
							/></label>
							<label
								>CO2 <input type="number" v-model.number="t.emissione"
							/></label>
						</div>
					</div>
						...
						<strong
							...
							{{
								Math.max(
									0,
									100 -
										tipologieAggiuntive.reduce((a, b) => a + b.percentuale, 0),
								)
							}}%</strong
						>
						<label
...
<script setup>
	...
	const initMap = async () => {
		...
		L.tileLayer(
			...
			{
				attribution:
					"Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EBP, and the GIS User Community",
			},
			...
			sensoriFissi.value.forEach((s) => {
				if (s.qta > 50) {
					L.circle([s.lat, s.lon], {
						...
						.bindPopup(`Zona Critica: ${s.qta} veicoli`);
				}
			});
		}
	};
	...
	const formatTime = (pct) => {
		const ms =
			dataInizio.getTime() +
			(pct / 100) * (dataFine.getTime() - dataInizio.getTime());
