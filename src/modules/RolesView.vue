<script setup>
	...
	const getNomiModuli = (moduliIds) => {
		...
		return moduliIds.map((id) => {
			const moduloTrovato = tuttiIModuli.value.find(
				(m) => extractId(m.self) === id,
			);
	...
	const selezionaRuolo = (r) => {
		...
		const isAdminCheck =
			r.nome.toLowerCase() === "admin" ||
			r.nome.toLowerCase() === "amministratore";
		...
		ruoloInModifica.value = {
			...
			moduli: isAdminCheck
				...
				: (r.moduli || []).filter((id) => {
						const mod = tuttiIModuli.value.find(
							(m) => extractId(m.self) === id,
						);
	...
	const salvaModifiche = async () => {
		...
		try {
			const url = isNew.value
				? "/api/roles"
				: `/api/roles/${ruoloInModifica.value.id}`;
			...
			if (res.ok) {
				...
				alert(
					"Errore durante il salvataggio: " +
						(errore.message || "Ruolo già presente o richiesta non valida"),
				);
			}
		} catch (error) {
	...
	<div class="dashboard-page">
		<header class="navbar-panel">
			...
			<div class="controls-group">
				<button @click="preparaNuovoRuolo" class="btn-primary">
					➕ Crea Nuovo Ruolo
				</button>
			</div>
		</header>
		...
		<div class="main-content">
					...
					<table @mouseleave="activeColIndex = null">
						<thead>
							<tr>
								...
								<th :class="{ 'on-hover': activeColIndex === 1 }">
									DESCRIZIONE
								</th>
							...
							>
								...
								<td @mouseover="handleMouseOver(2)">
									<div class="tags-container">
										<span
											v-for="mNome in getNomiModuli(r.moduli)"
											:key="mNome"
											class="tag-modulo"
										>
											{{ mNome }}
										...
										<span
											v-if="!r.moduli || r.moduli.length === 0"
											class="no-modules"
										>
			...
			<aside class="sidebar-panel panel bar" :class="{ open: barOpen }">
				<div v-if="ruoloInModifica" class="settings-content">
					...
					<div class="scroll-area form-container">
						...
						<div class="form-group">
							...
							<textarea
								v-model="ruoloInModifica.descrizione"
								rows="4"
								placeholder="Inserisci una descrizione per questo ruolo..."
							></textarea>
						</div>
						...
						<div class="form-group">
							...
							<p class="hint">
								Seleziona i moduli visibili per gli utenti con questo ruolo:
							</p>
							...
							<div class="modules-checkbox-list">
								<label
									...
									:class="{
										'disabled-item':
											isRoleAdmin ||
											(!isRoleAdmin && isModuloPrivilegiato(mod)),
									}"
								>
									<input
										type="checkbox"
										...
										:disabled="
											isRoleAdmin || (!isRoleAdmin && isModuloPrivilegiato(mod))
										"
									/>
									...
									<div class="checkbox-label-text">
										...
										<span class="mod-desc">
											<template
												v-if="!isRoleAdmin && isModuloPrivilegiato(mod)"
											>
				...
				<div v-else class="empty-state">
					Seleziona un ruolo dalla tabella per configurarlo o clicca su "Crea
					Nuovo Ruolo".
				</div>
			</aside>
		</div>
...
<style scoped>
	...
	.modules-checkbox-list::-webkit-scrollbar {
		width: 4px;
	}
	.modules-checkbox-list::-webkit-scrollbar-thumb {
		background: #a0aec0;
		border-radius: 4px;
	}
	.modules-checkbox-list::-webkit-scrollbar-track {
		background: transparent;
	}
	...
	.checkbox-item:hover:not(.disabled-item) {
		background: #f0fff4;
		border-color: #c6f6d5;
	}
