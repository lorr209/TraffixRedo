	...
	const salvaDati = async () => {
		try {
			const url = isNew.value
				? "/api/users"
				: `/api/users/${utenteInModifica.value.id}`;
			...
			if (isNew.value) {
				if (!utenteInModifica.value.password)
					return alert("Inserisci una password per il nuovo utente!");
	...
	const resetPassword = async () => {
		...
		try {
			const res = await fetch(
				`/api/users/${utenteInModifica.value.id}/password`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
	...
	<div class="dashboard-page">
		<header class="navbar-panel">
			...
			<div class="controls-group">
				<button @click="preparaNuovoUtente" class="btn-primary">
					+ Crea Nuovo Account
				</button>
			</div>
		</header>
						...
						<tbody>
							...
							>
								...
								<td @mouseover="activeColIndex = 4">
									<span
										:class="['pallino', u.attivo ? 'attivo' : 'disattivo']"
									></span>
			...
			<aside class="sidebar-panel panel bar" :class="{ open: barOpen }">
				<div v-if="utenteInModifica" class="settings-content">
					...
					<p v-if="!isNew" class="edit-subtitle">
						Modificando:
						<strong
							>{{ utenteInModifica.nome }}
							{{ utenteInModifica.cognome }}</strong
						>
					</p>
					...
					<div class="scroll-area form-container">
						...
						<div class="form-group">
							<label>Cognome</label>
							<input
								v-model="utenteInModifica.cognome"
								type="text"
						...
						<div class="form-group">
							<label>Email</label>
							<input
								v-model="utenteInModifica.email"
								type="email"
						...
						<div class="form-group" v-if="isNew">
							...
							<input
								v-model="utenteInModifica.password"
								type="text"
					...
					<div class="actions-grid">
						...
						<button v-if="!isNew" @click="resetPassword" class="btn-secondary">
							Reset Password
						</button>
				...
				<div v-else class="empty-state">
					Seleziona una riga dalla tabella per modificare l'utente oppure clicca
					su "Crea Nuovo Account".
				</div>
			</aside>
		</div>
	</div>
...
<style scoped>
	...
	.disattivo {
		...
		box-shadow: 0 0 5px rgba(245, 101, 101, 0.4);
	}
