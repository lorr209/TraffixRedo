	...
	<div class="dashboard-container">
			...
			<div class="table-wrapper">
				<table @mouseleave="activeColIndex = null">
					<thead>
						<tr>
							<th
								v-for="(h, i) in ['NOME', 'COGNOME', 'EMAIL', 'STATO']"
								:key="h"
		...
		<aside class="bar" :class="{ open: barOpen }">
			<div v-if="utenteInModifica" class="settings-content">
				...
				<p v-if="!isNew">
					Modificando: <strong>{{ utenteInModifica.nome }}</strong>
				</p>
				...
				<div class="actions-grid">
					<button
						@click="toggleStato"
						:class="
							utenteInModifica.stato === 'attivo' ? 'btn-warn' : 'btn-success'
						"
					>
						{{
							utenteInModifica.stato === "attivo"
								? "Disattiva Account"
								: "Attiva Account"
						}}
					</button>

					<button @click="resetPassword" class="btn-info" v-if="!isNew">
						Reset Password
					</button>
...
<script setup>
	...
	const toggleStato = () => {
		utenteInModifica.value.stato =
			utenteInModifica.value.stato === "attivo" ? "disattivo" : "attivo";
	};
