	...
	<section class="dashboard-layout">
								...
								<div class="visual-grid">
									...
									>
										<div
											class="bar-traffico"
											:style="{
												height: datiGrafico.datasets[0].data[i] + 'px',
											}"
										></div>
										<div
											class="bar-inquinamento"
											:style="{
												height: datiGrafico.datasets[1].data[i] + 'px',
											}"
										></div>
		...
		<aside class="control-panel">
			...
			<div class="scroll-area">
				<div
					v-for="tipo in tipologieAggiuntive"
					:key="tipo.id"
					class="tipo-item"
				>
					<div class="item-top">
						<input v-model="tipo.nome" class="input-name" />
						<button @click="rimuoviTipologia(tipo.id)" class="btn-del">
							×
						</button>
					</div>
...
<script setup>
	...
	const fetchData = async () => {
		trackingGPS.value = [
			{ id: "V1", lat: 45.4642, lon: 9.19, time: "10:00:00" },
			{ id: "V1", lat: 45.4645, lon: 9.191, time: "10:00:10" },
		];

		sensoriFissi.value = [
			{ id: "S_Duomo", lat: 45.4641, lon: 9.1919, qta: 120, time: "10:00:00" },
			{ id: "S_Centrale", lat: 45.485, lon: 9.203, qta: 45, time: "10:00:00" },
		];
	};
	...
	const densitaMedia = computed(() => {
		const rapporto =
			(trackingGPS.value.length / sensoriFissi.value.length) * 10;
	...
	const fattoreInquinamentoMedio = computed(() => {
		const percOccupata = tipologieAggiuntive.value.reduce(
			(acc, t) => acc + t.percentuale,
			0,
		);
